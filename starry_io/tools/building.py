...
def PrepareBuilding(env, root_directory, has_libcpu=False, remove_components = []):
    ...
    bsp_vdir = 'build/bsp'
    lib_vdir = 'build/lib'
    libc_vdir = 'build/libc'
    # board build script
    objs = SConscript('SConscript', variant_dir=bsp_vdir, duplicate=0)
    # include Libraries
    objs.extend(SConscript(Rtt_Root + '/Libraries/SConscript', variant_dir=lib_vdir, duplicate=0))
    # include libc
    objs.extend(SConscript(Rtt_Root + '/libc/SConscript', variant_dir=libc_vdir, duplicate=0))
