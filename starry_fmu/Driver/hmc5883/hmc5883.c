 ...
 * Change Logs:
 * Date           Author       Notes
 * 2017-10-19     zoujiachi    first version.
 */
...
int _hmc5883_collect(int16_t raw_mag[3])
{
	 ...
	 */
	if ((abs(raw_val[0]) > 2048) ||
	    (abs(raw_val[1]) > 2048) ||
	    (abs(raw_val[2]) > 2048)) {
		printf("hmc5883 collect error\n");
		return 1;
	}
...
rt_err_t hmc5883_init(rt_device_t dev)
{
	...
	if(_i2c_device != RT_NULL)
	{
		...
		if(res != RT_EOK){
			printf("hmc5883 i2c open fail\n");
			return res;
		}

		if(!_hmc5883_check_identity()){
			printf("hmc5883 identity check fail\n");
			return RT_ERROR;
		}
...
rt_size_t hmc5883_read(rt_device_t dev, rt_off_t pos, void *buffer, rt_size_t size)
{
	...
	if(pos == 1)	/* read raw mag data */
	{
		for(uint16_t i = 0 ; i<size ; i++)
		{
			res = _hmc5883_collect(((int16_t*)buffer)+i*3);
...
rt_err_t hmc5883_control(rt_device_t dev, rt_uint8_t cmd, void *args)
{
	rt_err_t res = RT_EOK;
